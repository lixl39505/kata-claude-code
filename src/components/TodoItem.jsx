...
function TodoItem({ task, provided, isDragging }) {
  const { dispatch } = useTodo()

  const getItemStyle = (draggableStyle) => ({
    ...draggableStyle,
    ...(draggableStyle?.transition && {
      transition: draggableStyle.transition.replace(/\d+\.?\d*s/, '0.2s'),
    }),
  })
  ...
  return (
    ...
    >
      <div
        className={`flex items-center justify-between gap-3 px-4 py-3 bg-white rounded-lg shadow-sm border border-gray-100 ${
